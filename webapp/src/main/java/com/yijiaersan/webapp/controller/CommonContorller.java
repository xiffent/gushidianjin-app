...
public class CommonContorller {
	...
	public ResultEntity<String> sendSms(String telphonenum) {
		...
		try {
			...
			//resultSms = "{\"alibaba_aliqin_fc_sms_num_send_response\": {\"result\": {\"err_code\": \"0\",\"model\": \"101637153751^1102180248323\",\"success\": true},\"request_id\": \"z2anxc5liyfl\"}}";
			if (!resultSms.equals("")) {
				Map aliResultMap = (Map) JSONObject.parseObject(resultSms).get("alibaba_aliqin_fc_sms_num_send_response");
			...
			sms.setTelphone(telphonenum);
			sms = smsCodeService.selectSmsCode(sms);*/
