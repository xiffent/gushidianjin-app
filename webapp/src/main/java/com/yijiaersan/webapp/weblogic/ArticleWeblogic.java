...
public class ArticleWeblogic
{
  ...
  public ResultEntity<String> insertArticleInfo(Long userId, String jsonData)
  {
    ...
    try {
      ...
      ResponseModel responseModel = (ResponseModel)JSONUtil.json2Bean(jsonData, 
        ResponseModel.class);
      ...
      List articleSharesRelations = responseModel.getArticleSharesRelations();

      List articleSharesRelationsNew = new ArrayList();

      for (int i = 0; i < articleSharesRelations.size(); ++i) {
        ArticleSharesRelation articleSharesRelation = (ArticleSharesRelation)articleSharesRelations.get(i);
      ...
      this.articleSharesRelationService.insertArticleSharesRelationsList(articleSharesRelationsNew);
  ...
  public ResultEntity<ResponseModel> queryArticleDetail(Long articleId)
  {
    ...
    ArticleType articleType = this.articleTypeService.selectArticleType(article
      .getArticleTypeId());
    if (articleType != null) {
  ...
  public List<ArticleList> queryList(List<Article> articles)
  {
    ...
    for (int i = 0; i < articles.size(); ++i) {
      ...
      List asr = this.articleSharesRelationService.selectArticleSharesRelation(article.getArticleId());

      for (int j = 0; j < asr.size(); ++j) {
        ArticleSharesRelations asrs = new ArticleSharesRelations();
        ArticleSharesRelation articleSharesRelation = (ArticleSharesRelation)asr.get(j);
      ...
      ArticleType articleType = this.articleTypeService.selectArticleType(article.getArticleTypeId());
