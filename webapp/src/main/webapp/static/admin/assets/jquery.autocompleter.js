...
;(function ($, window) {
            ...
            for (var i = 0; i < 2; i++) {
                for (var item in source) {
                    if (response.length < limit) {
                        switch (i) {
                            case 0:
                                if (source[item].sharesPy.toUpperCase().search(query) === 0) {
                            ...
                            case 1:
                                if (source[item].sharesPy.toUpperCase().search(query) !== -1) {
                                    response.push(source[item]);
                                    delete source[item];
                                }
                            break;
    ...
    function _buildList(list, data) {
        ...
        for (var item = 0, count = list.length; item < count; item++) {
            ...
            var label = (data.customLabel && list[item][data.customLabel]) ? list[item][data.customLabel] : list[item].sharesPy;
        ...
        if (list.length && data.hint) {
            var hint = ( list[0].sharesPy.substr(0, data.query.length).toUpperCase() === data.query.toUpperCase() ) ? list[0].sharesPy : false;
